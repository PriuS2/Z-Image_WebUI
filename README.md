...
**Z-Image Turbo + Qwen-Image-Edit 기반 다중 사용자 AI 이미지 생성/편집 웹 인터페이스**
...
Z-Image WebUI는 [Tongyi-MAI/Z-Image-Turbo](https://huggingface.co/Tongyi-MAI/Z-Image-Turbo) 이미지 생성 모델과 [ovedrive/Qwen-Image-Edit-2511-4bit](https://huggingface.co/ovedrive/Qwen-Image-Edit-2511-4bit) 편집 모델을 결합한 FastAPI 웹 애플리케이션입니다. 다중 사용자 지원, 한국어 프롬프트 지원, GGUF 양자화, 실시간 편집 기능 등 다양한 기능을 제공합니다.

### 왜 Z-Image WebUI인가?

- ⚡ **빠른 생성**: Turbo 모델로 4~12 스텝만에 고품질 이미지 생성
- 🎨 **실시간 편집**: Qwen-Image-Edit로 이미지 수정 및 향상
- 👥 **다중 사용자**: 회원가입/로그인 지원 및 사용자별 데이터 관리
- 🇰🇷 **한국어 지원**: 한국어 프롬프트 자동 번역 및 AI 향상
- 💾 **VRAM 절약**: GGUF 양자화로 3.8GB~7.2GB VRAM만으로 실행 가능
- 🔑 **API 지원**: 외부 API 키를 통한 접근 지원
...
### 생성 탭 - 이미지 생성
![생성 탭](Assets/01_webui.png)

### 편집 탭 - 이미지 수정
![편집 탭](Assets/edit_tab.png)

### 갤러리 탭
![갤러리](Assets/02_갤러리.png)

### 히스토리 탭
![히스토리](Assets/03_history.png)

### 즐겨찾기 탭
![즐겨찾기](Assets/04_favorite.png)

### 설정 탭
![설정](Assets/05_Settings.png)

### 관리자 패널 (localhost 전용)
![관리자](Assets/admin_panel.png)
...
- **게스트 모드**: 별도 가입 없이 바로 사용 가능
- **세션 관리**: 사용자별 독립적인 데이터 공간
- **API 키 지원**: 외부 애플리케이션 연동
...
- **실시간 생성**: WebSocket을 통한 진행 상황 실시간 표시

### 🎭 이미지 편집
- **Qwen-Image-Edit**: 고품질 이미지 수정 및 향상
- **다중 입력 지원**: 1~3장의 이미지 입력으로 복합 편집
- **True CFG Scale**: 향상된 생성 품질 제어
- **편집 히스토리**: 수정 과정 추적 및 복원
...
|----------|----------|----------|
| Python | 3.8+ | 3.10+ |
| VRAM | 8GB (생성+편집) | 16GB+ (고해상도 편집) |
| RAM | 16GB | 32GB |
| 저장 공간 | 30GB | 50GB |
| GPU | RTX 3060 이상 | RTX 4070 이상 |

**참고**: 생성 모델(Q3 양자화)만 사용할 경우 4GB VRAM으로도 실행 가능하나, 편집 기능은 최소 8GB VRAM 권장
...
1. **편집 탭**으로 이동
2. **이미지 업로드**: 편집할 이미지 선택 (1~3장)
3. **편집 프롬프트**: 수정할 내용 입력
4. **편집 설정**: 강도, 스텝 수 등 조정
5. **편집 시작** 버튼 클릭
...
5. 생성된 이미지를 **편집**으로 추가 수정
6. 좋은 프롬프트는 **즐겨찾기**에 저장
...
1. **로그인 필요**: 회원가입 또는 게스트 모드로 시작
2. **세션 관리**: 브라우저 세션별 데이터 격리
3. **API 키**: 외부 애플리케이션 연동용 키 생성 가능
...
| Ollama | 로컬, 무료 | 불필요 |
| Together AI | 다양한 오픈소스 모델 | [together.ai](https://together.ai) |

### GPU 및 모델 설정

- **생성 모델 GPU**: 이미지 생성에 사용할 디바이스 선택
- **편집 모델 GPU**: 이미지 편집에 사용할 디바이스 선택
- **자동 언로드**: VRAM 절약을 위한 자동 모델 언로드
  - 생성 모델 타임아웃: 기본 10분
  - 편집 모델 타임아웃: 기본 10분
...
├── routers/                  # API 라우터 모듈
│   ├── auth.py               # 인증 관련 API
│   ├── admin.py              # 관리자 API (localhost 전용)
│   ├── gallery.py            # 갤러리 API
│   ├── history.py            # 히스토리/즐겨찾기 API
│   ├── settings_router.py    # 설정 API
...
│   ├── qwen_edit.py          # Qwen 편집 모델 관리
│   ├── edit_history.py       # 편집 히스토리 관리
│   ├── edit_llm.py           # 편집용 LLM 도구
│   ├── queue_manager.py      # 생성 큐 관리
...
│   ├── settings.yaml         # 전역 설정
│   ├── sessions/             # 세션별 데이터
│   ├── history.json          # 레거시 히스토리
│   └── favorites.json        # 레거시 즐겨찾기
├── outputs/                  # 생성된 이미지 저장 (사용자별)
├── requirements.txt          # 메인 의존성
├── HowToUse.md               # 상세 사용 가이드
├── Setup.bat                 # Windows 설치 스크립트
├── Run.bat                   # Windows 실행 스크립트
└── Qwen_Image_Edit_MODEL_GUIDE.md  # 편집 모델 가이드
```
...
| `/api/login` | POST | 로그인 |
| `/api/guest` | POST | 게스트 로그인 |
| `/api/logout` | POST | 로그아웃 |
| `/api/me` | GET | 현재 사용자 정보 |
| `/api/change-password` | POST | 비밀번호 변경 |

#### 이미지 생성/편집 API
| 엔드포인트 | 메서드 | 설명 |
|-----------|-------|------|
| `/api/status` | GET | 시스템 상태 |
| `/api/model/load` | POST | 생성 모델 로드 |
| `/api/model/unload` | POST | 생성 모델 언로드 |
| `/api/generate` | POST | 이미지 생성 |
| `/api/edit/load` | POST | 편집 모델 로드 |
| `/api/edit/unload` | POST | 편집 모델 언로드 |
| `/api/edit` | POST | 이미지 편집 |
...
| `/api/edit/translate` | POST | 편집용 프롬프트 번역 |
| `/api/edit/enhance` | POST | 편집용 프롬프트 향상 |
| `/api/edit/suggest` | POST | 편집 제안 |
...
| `/api/history/{id}` | GET/PATCH | 히스토리 상세/수정 |
| `/api/favorites` | GET/POST | 즐겨찾기 목록/추가 |
| `/api/favorites/{id}` | DELETE | 즐겨찾기 삭제 |
| `/api/edit-history` | GET | 편집 히스토리 |

#### 관리자 API (localhost 전용)
| 엔드포인트 | 메서드 | 설명 |
|-----------|-------|------|
| `/api/admin/users` | GET | 사용자 목록 |
| `/api/admin/sessions` | GET | 세션 현황 |
| `/api/admin/gpu-status` | GET | GPU 상태 |
| `/api/admin/api-keys` | GET/POST | API 키 관리 |
...
- [ovedrive/Qwen-Image-Edit-2511-4bit](https://huggingface.co/ovedrive/Qwen-Image-Edit-2511-4bit) - 이미지 편집 모델
